...
- The Adventure of the Speckled Band by A. C. Doyle as a reference
- Clearly identify who the victim is and why the player is there
...
- No combination puzzles. Each item’s transitions are independent.
- States should reveal clues and game lore. Make them explicit so that the engine picks them up.
...
- Large, fixed, or infrastructural objects should be non-collectible.
- Informational sources (emails, logs, displays) are non-collectible.
- They change state only by EXAMINE or USE and remain in place.
...
- DO NOT use ACTION_TYPE_TAKE or ACTION_TYPE_DROP in transitions
- Provide only minimal transitions needed (most clue items are a single one-way transition from index 0 -> 1).
...
- Provide at least 2 independent victory condition sets (A, B, ...).
...
- Index 0 (GAME_STATE_CONTINUE): Empty.
- Index 1 (GAME_STATE_VICTORY): Explicitly enumerate why there was a success
- Index 2 (GAME_STATE_DEAD): Explicitly narrate why the player lost: what happened and why that is bad.
...
* Descriptions of items that you contain clues should not reveal details. Details should only be revealed when inspecting
